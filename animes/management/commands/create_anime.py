...
class Command(BaseCommand):
    ...
    def handle(self, *args,**options):
        ...
        for anime in data.json():
            ...
            anime_episodes : list = httpx.get(f'{self.ANIME_API}{self.query_id}{id}').json()
            if(not anime_episodes):
                continue
            for episode in anime_episodes:
                episode_id = episode['video_id']
                name = episode['title']
                anime_episode = httpx.get(f'{self.ANIME_API}{self.query_episode}{episode_id}').json()[0]
